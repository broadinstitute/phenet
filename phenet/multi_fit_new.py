...
parser.add_option("","--min-traits",type="int",default=2) #minimum number of non-missing traits required for variants to be used in training
parser.add_option("","--mcmc-samp-iter",type="int",default=1000) #number of samplings each iteration of training algorithm to estimate expected values
parser.add_option("","--mcmc-samp-burn",type="int",default=100) #number of samplings each iteration of training algorithm that are thrown away as burn-in
parser.add_option("","--mcmc-samp-thin",type="int",default=10) #fraction of samplings each iteration to keep
parser.add_option("","--train-eps",type="float",default=1e-3) #difference in log-likelihoods at which to terminate training
parser.add_option("","--max-train-it",type="int",default=100) #maximum number of training iterations to use, even if convergence is not reached
parser.add_option("","--no-normalize",action="store_false",dest="normalize",default="True") #maximum number of training iterations to use, even if convergence is not reached
...
if train:
    for trait in traits:
        ...
        if EFFECT_COL not in traits[trait] and (FIXED not in traits[trait] or need_fixed not in traits[trait][FIXED] or need_fixed not in traits[trait]):
            bail("Error: please specify a value for %s for trait %s in the config file, or assign %s a value and specify it as fixed for %s" % (EFFECT_COL, trait, need_fixed, trait))
        if SE_COL not in traits[trait] and P_COL not in traits[trait] and (FIXED not in traits[trait] or need_fixed not in traits[trait][FIXED] or need_fixed not in traits[trait]):
            bail("Error: please specify a value for either %s or %s for trait %s in the config file, or assign %s a value and specify it as fixed for %s" % (SE_COL, P_COL, trait, need_fixed, trait))

if classify:
    for trait in traits:
        if (EFFECT_COL in traits[trait] and (SE_COL not in traits[trait] and P_COL not in traits[trait])) or (SE_COL in traits[trait] and EFFECT_COL not in traits[trait]):
            warn("Both %s and (%s or %s) are required for %s; specifying only one will cause it to be treated as missing" % (EFFECT_COL, SE_COL, P_COL, trait))
    ...
    for info in needed_columns:
        ...
        for i in range(len(traits[trait][FILE])):
            if traits[trait][info] not in traits[trait][COL_MAP][i]:
                bail("Error: file %s for trait %s does not have column %s" % (traits[trait][FILE][i], trait, traits[trait][info]))
            ...
            for i in range(len(traits[trait][FH])):
                ...
                for line in traits[trait][FH][i]:
                    ...
                    if not len(cols) == len(traits[trait][COL_MAP][i]):
                        warn("Skipping line; number of columns (%s) doesn't match header (%s): %s" % (len(cols), len(traits[trait][COL_MAP][i]), line))
...
if train:
    ...
    for it in range(options.max_train_it):
        ...
        for var_id in variants:
            ...
            for name in traits:
                if name in variants[var_id]:
                    model_traits.append(mpheno.Trait(name, beta=traits[name][OBJ].beta, var=traits[name][OBJ].var, effect=variants[var_id][name].effect, se=variants[var_id][name].se))
            ...
            if it > 0:
                for endo in endos:
                    ...
                    for trait in traits:
                        ...
                        new_log_lik += M.get_normal_like(mean_trait, traits[trait][OBJ].beta * mean_endo, traits[trait][OBJ].var)
                for trait in traits:
                    if trait not in M.get_node_names():
                        continue
                    trait_values = M.get_trace(trait)
                    mean_trait = np.mean(trait_values)
                    new_log_lik += M.get_normal_like(variants[var_id][trait].effect, mean_trait, variants[var_id][trait].se**2)
        ...
        for endo in endos:
            ...
            if FIXED not in endos[endo] or VAR not in endos[endo][FIXED]:
                endos[endo][OBJ].var = (endo2_tot[endo] - 2 * endos[endo][OBJ].mean * endo_tot[endo] + endo_n[endo] * endos[endo][OBJ].mean**2) / endo_n[endo]
            ...
            for trait in traits:
                ...
                if FIXED not in traits[trait] or VAR not in traits[trait][FIXED]:
                    traits[trait][OBJ].var = (trait2_tot[trait] - 2 * traits[trait][OBJ].beta * endo_x_trait_tot[endo][trait] + traits[trait][OBJ].beta**2 * endo2_for_trait_tot[endo][trait]) / trait_n[trait]
                    if traits[trait][OBJ].var < 0:
                        traits[trait][OBJ].var = 0

                log("%s: beta=%s, mean=%s, var=%s" % (trait, traits[trait][OBJ].beta, traits[trait][OBJ].beta * endos[endo][OBJ].mean, traits[trait][OBJ].var), TRACE)
    ...
    for trait in traits:
        log("%s: beta=%s, mean=%s, var=%s" % (trait, traits[trait][OBJ].beta, traits[trait][OBJ].beta * endos[endo][OBJ].mean, traits[trait][OBJ].var), DEBUG)
        ...
        for name in traits:
            ...
            if len(values) == 2:
                if 1 in values and 0 in values:
                    ...
                    warn("Trait %s has only two values (%s) but they are not 0/1 or 1/2. Treating as quantitative" % (name, values))
    ...
    for var_id in variants_to_write:
        ...
        if var_id in variants:
            ...
            for name in traits:
                ...
                model_traits.append(mpheno.Trait(name, beta=traits[name][OBJ].beta, var=traits[name][OBJ].var, effect=effect, se=se, dichotomous=(name in dichotomous_traits), baseline=baseline))
