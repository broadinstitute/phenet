            ...
            with self.M:
                ...
                for trait in traits:
                    ...
                    if trait.dichotomous or not trait.se == 0:
                        trait_nodes[trait.name] = self.pymc.Normal(trait.name, mu=mult_endo_nodes[trait.name], sd=cur_sd)
                        if trait.dichotomous:
                            ...
                            obs_nodes[self.obs_name(trait.name)] = self.pymc.Bernoulli(self.obs_name(trait.name), p=p, observed=trait.effect)
                        elif trait.effect is not None and trait.se is not None:
                            obs_value = trait.effect
                            if trait.baseline is not None:
                                obs_value = trait.effect - trait.baseline
                            obs_nodes[self.obs_name(trait.name)] = self.pymc.Normal(self.obs_name(trait.name), mu=trait_nodes[trait.name], tau=1/(trait.se)**2, observed=obs_value)
                    else:
                        trait_nodes[trait.name] = self.pymc.Normal(trait.name, mu=mult_endo_nodes[trait.name], sd=cur_sd, observed=trait.effect)
                    ...
                    if not trait.se == 0:
                        ...
                        if trait.dichotomous:
                            ...
                            if trait.baseline is not None:
                                b_o = np.log(trait.baseline/(1-trait.baseline))
                            p = 1 / (1 + np.exp(-b_o - trait_nodes[trait.name]))
                            obs_nodes[self.obs_name(trait.name)] = self.pymc.Bernoulli(self.obs_name(trait.name), p=p, value=trait.effect, observed=True)
                        elif trait.effect is not None and trait.se is not None:
                            obs_value = trait.effect
                            if trait.baseline is not None:
                                obs_value = trait.effect - trait.baseline
                            obs_nodes[self.obs_name(trait.name)] = self.pymc.Normal(self.obs_name(trait.name),mu=trait_nodes[trait.name], tau=1/(trait.se)**2, value=obs_value, observed=True)
                    else:
                        trait_nodes[trait.name] = self.pymc.Normal(trait.name, mu=mult_endo, tau=cur_tau, value=trait.effect, observed=True)
                    return locals()
    ...
    def get_node_names(self):
        if self.node_names is None:
            if self.use_pymc3:
                ...
                self.node_names = set(self.MAP.endo_nodes.keys() + self.MAP.trait_nodes.keys() + self.MAP.obs_nodes.keys())
        ...
        for i in range(max_ests):
            ...
            if cur_logp > max_logp:
                ...
                if self.use_pymc3:
                    ...
                    self.log("New maximum: %s; saw previous %s times" % (self.MAP.endo_nodes[endo].value, times_seen), DEBUG)
                ...
                if times_seen >= times_seen_break:
                    if self.use_pymc3:
                        for endo in self.endo_names:
                            self.log("Early termination at iteration %s for %s: %s" % (i+1, endo, best_map_value[endo]), DEBUG)
                    else:
                        for endo in self.MAP.endo_nodes:
                            self.log("Early termination at iteration %s for %s: %s" % (i+1, endo, self.MAP.endo_nodes[endo].value), DEBUG)
                    break
